...
public class DifferentWindowOnMainMenu : MonoBehaviour
{
    ...
    public void Show(Button button = null)
    {
        ...
        if (button != null)
            _tween
                .Append(button.gameObject.transform.DOScale(0.0f, 0.2f).From(1.5f).SetEase(Ease.InBounce));

        _tween
            .Append(gameObject.transform.DOMoveY(_startPosition.y, 0.5f).From(_endPosition.y).SetEase(Ease.Linear))
            .Join(_canvasGroup.DOFade(1.0f, 0.5f).From(0.0f));        
    }

    public void Hide(Button button = null)
    {
        if (_canvasGroup.alpha > 0.0f)
        {
            ...
            if (button != null)
                _tween
                    .Append(button.gameObject.transform.DOScale(1.5f, 0.2f).From(0.0f).SetEase(Ease.OutBounce));
